...
[Python 版本](https://www.python.org/downloads/)
[最后提交](https://github.com/agentscope-ai/CoPaw)
[许可证](LICENSE)
[代码风格](https://github.com/psf/black)
[GitHub Star](https://github.com/agentscope-ai/CoPaw/stargazers)
[GitHub Fork](https://github.com/agentscope-ai/CoPaw/network)
[DeepWiki](https://deepwiki.com/agentscope-ai/CoPaw)
...
在浏览器打开 **[http://127.0.0.1:8088/](http://127.0.0.1:8088/)** 即可使用控制台（与 CoPaw 对话、配置 Agent）。若要在钉钉、飞书、QQ 等 app 内对话，请参考 [文档](https://copaw.agentscope.io/docs/channels) 接入频道。

![Console](https://img.alicdn.com/imgextra/i3/O1CN01N6TeJ41Y2y7O4gppz_!!6000000003002-2-tps-3328-1860.png)

### 脚本安装

无需手动配置 Python，一行命令自动完成安装。脚本会自动下载 uv（Python 包管理器）、创建虚拟环境f、安装 CoPaw 及其依赖（含 Node.js 和前端资源）。注意：部分网络环境或企业权限管控下可能无法使用。
...
然后在浏览器打开 **[http://127.0.0.1:8088/](http://127.0.0.1:8088/)** 进入控制台。配置、记忆与 Skills 保存在 `copaw-data` 卷中；模型配置与 API Key 保存在 `copaw-secrets` 卷中。如需传入 API Key（如 `DASHSCOPE_API_KEY`），在 `docker run` 时添加 `-e VAR=value` 或 `--env-file .env`。
...
1. **控制台（推荐）** — 运行 `copaw app` 后，打开 **[http://127.0.0.1:8088/](http://127.0.0.1:8088/)** → **设置** → **模型**。选择提供商、填写 **API Key**，并启用该提供商与模型。
