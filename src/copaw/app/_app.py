...
async def lifespan(
    ...
    try:
        ...
        # Stop GridPaw providers watcher (before multi-agent shutdown)
        prov_w = getattr(app.state, "providers_watcher", None)
        if prov_w is not None:
            try:
                await prov_w.stop()
            except Exception as e:
                logger.warning("Error stopping providers watcher: %s", e)
        ...
        if multi_agent_mgr is not None:
            ...
            try:
                ...
                logger.error("Error stopping MultiAgentManager: %s", e)
